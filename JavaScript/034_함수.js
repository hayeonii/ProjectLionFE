...
안녕() // hello hello hello - return 만나면 반환되기 때문에
...
// 함수를 사용하는 이유
    // 1. 재사용성 -> 재사용하지 않을 코드는 함수로 정의하나요? -> 가능하면 함수로 만드는 것 권장
...
함수1(10, 20, 30) // 콘솔 창의 기능, 마지막 라인에 한하여 console.log()를 찍지 않아도 return값 console창에 출력
...
function factorial(n) {
    if (n <= 1) { // 종료조건 - 종료조건이 없으면 무한 반복이 된다
        return 1
    }
...
// 함수가 종료된 다음에 선언된 변수는 휘발된다(메모리가 반환된다)
function xplus() {
    let x = 100;
    x = x + 10
} 
